...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Map projects to have a default status if missing
  const [projects, setProjects] = useState<Project[]>(PROJECTS.map(p => ({...p, status: p.status || 'done'})));
  ...
  const resetToDefaults = () => {
    ...
    setProjects(PROJECTS.map(p => ({...p, status: 'done'})));
    setSkills(SKILLS);
  ...
  return (
    <DataContext.Provider value={{
      ...
      addSkill, deleteSkill, addTestimonial, deleteTestimonial, addMeeting, deleteMeeting,
